...
def parse_keywords(user_input):
    """
    智能解析用户输入的关键词，支持多种分隔符
    
    Args:
        user_input: 用户输入（字符串或列表）
    ...
    Examples:
        ...
        "AI、machine learning、crypto" → ["AI", "machine learning", "crypto"]
        ["AI", "machine learning"] → ["AI", "machine learning"]
    """
    # 如果已经是列表，直接返回
    ...
    if not isinstance(user_input, str):
        raise ValidationError(f"❌ 关键词格式错误，必须是字符串或列表")
    ...
    for separator in ['、', ',', '\n', ';']:
        if separator in text:
            keywords = [kw.strip() for kw in text.split(separator)]
            break
...
def validate_keywords_list(keywords, field_type='keyword'):
    ...
    Args:
        keywords: 关键词或关键词列表（会先经过 parse_keywords 解析）
        field_type: 字段类型
