...
def update_marketplace_json(skill_name, skill_dir, root_dir):
    """
    Update the .claude-plugin/marketplace.json file with the new skill.
    
    Args:
        skill_name: Name of the skill
        skill_dir: Path to the skill directory
        root_dir: Root directory of the project (where .claude-plugin is located)
    ...
    if not marketplace_path.exists():
        print(f"⚠️  Warning: marketplace.json not found at {marketplace_path}")
        print("   Skipping marketplace.json update")
        return False
    ...
    try:
        ...
        # Calculate relative path from root_dir to skill_dir
        try:
            relative_path = skill_dir.relative_to(root_dir)
            source_path = f"./{relative_path}"
        except ValueError:
            # If skill_dir is not relative to root_dir, use absolute path
        ...
        new_plugin = {
            "name": skill_name,
            "description": f"{skill_title} skill",
            "source": source_path,
        ...
        if skill_name in plugin_names:
            print(f"⚠️  Plugin '{skill_name}' already exists in marketplace.json")
            return False
        ...
        # Write back to marketplace.json
        with open(marketplace_path, 'w', encoding='utf-8') as f:
            json.dump(marketplace_data, f, indent=2, ensure_ascii=False)
        
        print(f"✅ Updated marketplace.json with new plugin: {skill_name}")
        return True
...
def init_skill(skill_name, path):
    ...
    # Determine root directory (search upwards for .claude-plugin folder)
    ...
    for parent in [current_dir] + list(current_dir.parents):
        ...
        # If not found, use the parent of the skill directory
        root_dir = current_dir
    ...
    update_marketplace_json(skill_name, skill_dir, root_dir)
