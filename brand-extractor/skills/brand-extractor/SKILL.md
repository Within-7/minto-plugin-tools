...
| `{brand}_traffic_model.png` | 流量模型流程图 | 流程图 |
...
**字段说明**：
- `brandNumber` `brandName` `logoPath` - 品牌基本信息
- `userPositionImagePath` - 用户定位图片路径
- `userNeeds` - 用户需求（4个字段）
- `personaImage1Path` `personaImage2Path` - 用户画像图片路径
- `flowModelImagePath` - 流量模型图片路径
- `seoTraffic.text` - SEO说明文字（支持HTML）
- `seoTraffic.keywords` - 关键词数组（**必须包含 type 字段**）
- `backlinkTypes` - 外链类型文字说明
- `backlinkKeywords` - 外链关键词文字说明
...
**注意**：图片和数据**分开存放**，图片在 `assets/brand_analysis/`，JSON在 `data/`
...
### 核心文档
- [提取规则总结](references/extraction_rules.md) - 四种模式的输出规范
- [图片识别精确规则](references/image_identification_rules.md) - ⭐ 详细的图片识别逻辑
- [自检清单](references/checklist.md) - ⭐ 每个阶段的检查项

### 参考示例
- [完整提取示例](references/examples/complete_example.md) - ⭐ 端到端流程演示
- [布局参考目录](references/layouts/) - 15个布局参考文件

### 辅助工具
- [Slide分析脚本](../../scripts/slide_analyzer.py) - ⭐ 自动分析slide布局
...
### Q4：PPT结构与预期不同怎么办？
1. 先扫描所有slide文本，确认PPT的实际内容结构
2. 如果关键词不匹配，尝试使用更通用的关键词
3. 如果图片数量与预期不符，根据实际内容灵活调整
4. 与用户确认后再执行提取
